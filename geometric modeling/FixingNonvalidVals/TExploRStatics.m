...
% staticPoses = labCodeObj.computeStaticPoses();staticPoses = labCodeObj.computeStaticPoses();
...
%             zB = zB/norm(zB);
        ...
        function f = forces(obj, theta1, theta2, phi1, phi2, state, force) % Calculate forces for case 2
            Fr1plusFr2 = 2*(obj.M+obj.m)*obj.gc;
            Fr1minusFr2c2 = @(theta1,theta2,phi1,phi2,M,gc)  2*M*gc*((sin(theta1)-sin(theta2))/(sin(phi1))-cos(theta2)*cos(phi2));
            Fr1minusFr2 = Fr1minusFr2c2(obj.theta1,obj.theta2,obj.staticPoses.Case(state).phi1,obj.staticPoses.Case(state).phi2,obj.M,obj.gc);

            if (force == 1) % Fr1 value
                f = (Fr1plusFr2+Fr1minusFr2)/2;
            elseif (force == 2) % Fr2 value
                f = (Fr1plusFr2-Fr1minusFr2)/2;
            end
        end
    end
...
%             1 - Relationship between the two arc
            obj.T12 = [0,0,1,0; 0,-1,0,0.06; 1, 0, 0, 0; 0, 0, 0, 1]; % Transformation matrix between the two arcs
...
%             2 - Other robot parameters
        ...
        function staticPoses = computeStaticPoses(obj)
            ...
            for ii = 1:4    
                ...
                if (ii == 1)
                    ...
                    %obj.staticPoses.Case(ii).phi1 = obj.wrapTozero2Pi(atan((sin(obj.theta1) - sin(obj.theta2)) / cos(obj.theta1)));
                    obj.staticPoses.Case(ii).phi2 = 0;
                    %obj.staticPoses.Case(ii).zB = -[cos(obj.staticPoses.Case(ii).phi1), sin(obj.staticPoses.Case(ii).phi1), 1]'/sqrt(2);
                    
                    %Commented zB is what we've calculated previously.
                    %New zB below is for sensitivity analysis and is the
                    %output of changing T12 to match physical prototype
                    %obj.staticPoses.Case(ii).zB = -[cos(obj.staticPoses.Case(ii).phi1), sin(obj.staticPoses.Case(ii).phi1), 1]'/sqrt(2);
                    %obj.staticPoses.Case(ii).zB = [-50*obj.rB*cos(obj.staticPoses.Case(ii).phi1), -50*obj.rB*sin(obj.staticPoses.Case(ii).phi1), -(50*obj.rB-3*sin(obj.staticPoses.Case(ii).phi1))]'/sqrt(5000*obj.rB^2 - 300*obj.rB*sin(obj.staticPoses.Case(ii).phi1)+9*sin(obj.staticPoses.Case(ii).phi1)^2);
                elseif (ii == 2)   
                ...
                if (ii == 3)
                    ...
                    %obj.staticPoses.Case(ii).phi2 = obj.wrapTozero2Pi(atan((sin(obj.theta2) - sin(obj.theta1)) / cos(obj.theta2)));
                    %obj.staticPoses.Case(ii).zB = [-1, sin(obj.staticPoses.Case(ii).phi2), -cos(obj.staticPoses.Case(ii).phi2)]'/sqrt(2);
                    
                    %Commented zB is what we've calculated previously.
                    %New zB below is for sensitivity analysis and is the
                    %output of changing T12 to match physical prototype
                    %obj.staticPoses.Case(ii).zB = [-1, sin(obj.staticPoses.Case(ii).phi2), -cos(obj.staticPoses.Case(ii).phi2)]'/sqrt(2);
                    %obj.staticPoses.Case(ii).zB = [-(50*obj.rB - 3*sin(obj.staticPoses.Case(ii).phi2)), 50*obj.rB*sin(obj.staticPoses.Case(ii).phi2), -50*obj.rB*cos(obj.staticPoses.Case(ii).phi2)]'/sqrt(5000*obj.rB^2 - 300*obj.rB*sin(obj.staticPoses.Case(ii).phi2)+9*sin(obj.staticPoses.Case(ii).phi2)^2);
                elseif (ii == 4)
                ...
                if (ii == 1) % Calculate forces
                    %obj.staticPoses.Case(ii).Fr1 = obj.forces(obj, obj.theta1, obj.theta2, obj.staticPoses.Case(ii).phi1, obj.staticPoses.Case(ii).phi2, ii, 1);
                    %obj.staticPoses.Case(ii).Fr2 = obj.forces(obj, obj.theta1, obj.theta2, obj.staticPoses.Case(ii).phi1, obj.staticPoses.Case(ii).phi2, ii, 2);
                    %obj.staticPoses.Case(ii).Fr1 = (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 + obj.theta2))/2 - obj.m*obj.gc - obj.M*obj.gc - obj.M*obj.gc*cos(obj.theta2) + obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta1) - (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta2))/2;
                    %obj.staticPoses.Case(ii).Fr2 = (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta2))/2 - obj.m*obj.gc - obj.M*obj.gc + obj.M*obj.gc*cos(obj.theta2) - obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta1) - (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 + obj.theta2))/2;
                elseif (ii == 2)
                    %obj.staticPoses.Case(ii).Fr1 = (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 + obj.theta2))/2 - obj.m*obj.gc - obj.M*obj.gc + obj.M*obj.gc*cos(obj.theta2) + obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta1) - (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta2))/2;
                    %obj.staticPoses.Case(ii).Fr2 = (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta2))/2 - obj.m*obj.gc - obj.M*obj.gc - obj.M*obj.gc*cos(obj.theta2) - obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 - obj.theta1) - (obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi1 + obj.theta2))/2;
                elseif (ii == 3)
                    %obj.staticPoses.Case(ii).Fr1 = obj.M*obj.gc + obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi2)*cos(obj.theta1) - (obj.M*obj.gc*(sin(obj.theta2) - sin(obj.theta1)))/sin(obj.staticPoses.Case(ii).phi2)...
                    %                        + (obj.m*obj.gc);
                    %obj.staticPoses.Case(ii).Fr2 = obj.M*obj.gc - obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi2)*cos(obj.theta1) + (obj.M*obj.gc*(sin(obj.theta2) - sin(obj.theta1)))/sin(obj.staticPoses.Case(ii).phi2)...
                    %                        + (obj.m*obj.gc);
                elseif (ii == 4)
                    %obj.staticPoses.Case(ii).Fr1 = obj.M*obj.gc + obj.M*obj.gc*cos(obj.staticPoses.Case(ii).phi2)*cos(obj.theta1) - (obj.M*obj.gc*(sin(obj.theta2) - sin(obj.theta1)))/sin(obj.staticPoses.Case(ii).phi2)...
                    %                        + (obj.m*obj.gc);
                ...
                %obj.staticPoses.Case(ii).psb = [0; 0; -obj.staticPoses.Case2(ii).zB' * obj.staticPoses.Case2(ii).qB1];
                obj.staticPoses.Case(ii).Tsb = [obj.staticPoses.Case(ii).Rsb, obj.staticPoses.Case(ii).psb; 0, 0, 0, 1];
                nexttile()
                
                % p1 = strip(string(round(rad2deg(obj.staticPoses.Case(ii).phi1))));
                % p2 = strip(string(round(rad2deg(obj.staticPoses.Case(ii).phi2))));
                ...
                %title(['Case-',num2str(ii),', \phi_1=',num2str(round(rad2deg(obj.staticPoses.Case(ii).phi1))) char(176),', \phi_2=',num2str(round(rad2deg(obj.staticPoses.Case(ii).phi2))) char(176),', F_{R1}=',num2str(obj.staticPoses.Case(ii).Fr1),', F_{R2}=',num2str(obj.staticPoses.Case(ii).Fr2)]);
                %title(['Case ',num2str(ii),', \phi_1=',num2str(round(rad2deg(obj.staticPoses.Case(ii).phi1))) char(176),', \phi_2=',num2str(round(rad2deg(obj.staticPoses.Case(ii).phi2))) char(176)]);
                title({['Case ',num2str(ii),', \phi_1=',num2str(p1) char(176),', \phi_2=',num2str(p2) char(176)]});
                obj.iter = obj.iter + 1;
            end
            ...
            %title(['\theta_1=',num2str(rad2deg(obj.theta1)),'deg, \theta_2=',num2str(rad2deg(obj.theta2)),'deg']);
        end
        ...
        function drawTexplor(obj, Tsb, theta1, theta2, phi1, phi2, zB, Fr1, Fr2, iter)
            ...
            ann = annotation('textbox',pos,'String',annstr,'FitBoxToText','on', 'HorizontalAlignment',"center");
